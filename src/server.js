...
app.get('/api/terminal/:id/wait', async (req, res) => {
  const timeout = parseInt(req.query.timeout || '12000', 10);
  const result = await terminal.waitForOutput(req.params.id, timeout);
  res.json(result);
});
